...
bool Movable(AI* ai, float x, float y) {
	for (int i = floor(x + 0.01); i <= ceil(x - 0.01); i++) {
		for (int j = floor(y + 0.01); j <= ceil(y - 0.01); j++) {
			...
			if (b != BLOCK_GROUND) return false;
		}
	}
	return true;
}

void Change(AI* ai, float x, float y, float x2, float y2, float change) {
	for (int i = floor(x + 0.01); i <= ceil(x2 - 0.01); i++) {
		for (int j = floor(y + 0.01); j <= ceil(y2 - 0.01); j++) {
			if (i < 0 || j < 0 || i>21 || j>21) continue;
			if (!Movable(ai, i, j)) continue;
			goal[i][j] += change;
		}
	}
}
...
bool Shootable(AI* ai, float x, float y) {
	for (int i = floor(x + 0.01); i <= ceil(x - 0.01); i++) {
		for (int j = floor(y + 0.01); j <= ceil(y - 0.01); j++) {
			if (i < 0 || j < 0 || i>21 || j>21) return false;
			int b = ai->GetBlock(i, j);
...
bool collision(AI* ai, float x, float y, int id) {
...
int bulletCol(AI* ai, float x, float y, int t) {
	int pen = 0;

	hitbox thb(x - 0.55, y - 0.55, x + 0.55, y + 0.55);
	for (auto b : ai->GetEnemyBullets()) {
		float bx = b->GetX() + t * dx[b->GetDirection()] * b->GetSpeed();
		float by = b->GetY() + t * dy[b->GetDirection()] * b->GetSpeed();

		hitbox bhb(bx, by, bx - dx[b->GetDirection()] * b->GetSpeed() *0.6, by - dy[b->GetDirection()] * b->GetSpeed() *0.6);
...
int castRay(AI* ai, float x, float y,int dir, int offset, int start, int decay, bool tank = false) {
	int dist = 0;
	int cur = start;
...
int trigger(AI* ai, int id) {
	...
	for (int i = 1; i < 5; i++) {
		int dist = castRay(ai, tank->GetX(), tank->GetY(), i, 0, 0, 0);
		...
		for (int j = 0; j < 4;j++) {
			...
			float d = abs(tank->GetX() - e->GetX()) * dx[i] + abs(tank->GetY() - e->GetY()) * dy[i];

			float t = d / speed;
			
			hitbox temp = toHitbox(e->GetX()-0.5, e->GetY()-0.5, 1, 1, e->GetDirection(), min(2,max(0,t-0.9*e->GetSpeed())));
...
void calculate(AI* ai, int id) {
	...
	int offset = ceil(1.f / tank->GetSpeed()+ tank->GetSpeed());

	for (int i = 0; i < NUMBER_OF_TANK;i++) {
		auto t = ai->GetEnemyTank(i);
		int x = round(t->GetX());
		int y = round(t->GetY());
		Change(ai, t->GetX(), t->GetY(), t->GetX(), t->GetY(), -5000);

		if (t->GetHP() <= 0) continue;

		int start = 600+ (tank->GetHP()>t->GetHP())*5;

		castRay(ai, x, y-1, 1, offset, start, 50, true);
		castRay(ai, x+1, y, 2, offset, start, 50, true);
		castRay(ai, x, y+1, 3, offset, start, 50, true);
		castRay(ai, x-1, y, 4, offset, start, 50, true);
	}
	...
	for (int i = 0; i < NUMBER_OF_TANK; i++) {
		...
		castRay(ai, x, y-1, 1, 0, start, 1, true);
		castRay(ai, x+1, y, 2, 0, start, 1, true);
		castRay(ai, x, y+1, 3, 0, start, 1, true);
		castRay(ai, x-1, y, 4, 0, start, 1, true);
	}

	for (auto b : ai->GetEnemyBullets()) {
		castRay(ai, b->GetX(), b->GetY(), b->GetDirection(), 0, -b->GetDamage() * 50, -100);
	}

	for (auto p : ai->GetPowerUpList()) {
		Change(ai, p->GetX(), p->GetY(), p->GetX(), p->GetY(), 5000);
	}

	for (auto s : ai->GetIncomingStrike()) {
		Change(ai, s->GetX()-3, s->GetY()-3, s->GetX()+3, s->GetY()+3, -10000);
	}
...
int move(AI* ai, int id) {
	...
	for (int i = 0; i < 5; i++) {
		...
		float g = speed+speed*( (collision(ai, gx, gy, id) ? 5000 : 0) + bulletCol(ai, gx, gy, 1));
		float h = 1.01 * (abs(gx - tx) + abs(gy - ty));
	...
	while (!pq.empty()) {
		...
		for (int i = 0; i < 5; i++) {
			...
			float g = n.g + speed + speed*( (collision(ai, gx, gy, id) ? 2000 : 0) + bulletCol(ai, gx, gy, t));
			float h = 1.01 * (abs(gx - tx) + abs(gy - ty));
